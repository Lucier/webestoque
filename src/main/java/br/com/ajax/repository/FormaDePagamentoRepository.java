...
public class FormaDePagamentoRepository implements Serializable {
	...
	public List<FormaDePagamento> formasDePagamento(){
		return this.manger.createQuery("from FormaDePagamento", FormaDePagamento.class)
				.getResultList();
	}
	...
	public FormaDePagamento buscarPorDescricao(String descricao){
		try{
			return this.manger.createQuery("from FormaDePagamento where upper(descricao) = :descricao", FormaDePagamento.class)
	...
	public List<FormaDePagamento> formasDePagamentoFiltradas(FormaDePagamentoFilter formaDePagamentoFilter){
		...
		if(StringUtils.isNotBlank(formaDePagamentoFilter.getDescricao())){
			criteria.add(Restrictions.ilike("descricao", formaDePagamentoFilter.getDescricao(),
					MatchMode.ANYWHERE));
		}
